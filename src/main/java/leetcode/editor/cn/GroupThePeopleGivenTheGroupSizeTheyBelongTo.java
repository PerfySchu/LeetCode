...
class Solution {
    public List<List<Integer>> groupThePeople(int[] groupSizes) {
        ...
        for (int i = 0; i < groupSizes.length; i++) {
            List<Integer> list = sizeMap.getOrDefault(groupSizes[i], new ArrayList<>());
            list.add(i);
            sizeMap.put(groupSizes[i], list);
        }
        ...
        sizeMap.forEach((k, v) -> {
            ...
            for (int i = 0; i < size; i++) {
                ...
                list.addAll(v.subList(i*k, (i+1) * k));
                ans.add(list);
            }
        });
