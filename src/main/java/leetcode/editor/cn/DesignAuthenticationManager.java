...
public class DesignAuthenticationManager{
    public static void main(String[] args){
        AuthenticationManager solution = new DesignAuthenticationManager().new AuthenticationManager(5);
        solution.renew("aaa", 1);
        solution.generate("aaa", 2);
        System.out.println(solution.countUnexpiredTokens(6));
        solution.generate("bbb",7);
        solution.renew("aaa", 8);
        solution.renew("bbb", 7);
        System.out.println(solution.countUnexpiredTokens(15));
...
class AuthenticationManager {
    ...
    public void generate(String tokenId, int currentTime) {
        msgMap.put(tokenId, currentTime+timeout);
    }

    public void renew(String tokenId, int currentTime) {
        if (msgMap.getOrDefault(tokenId, 0) <= currentTime) {
            return;
        }
    ...
    public int countUnexpiredTokens(int currentTime) {
        return (int) msgMap.values().stream().filter(time -> time > currentTime).count();
    }
}
